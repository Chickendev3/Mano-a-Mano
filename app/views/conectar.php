  ...
  <div class="container">
    ...
    <div style="background-color: var(--color-surface); padding: 24px; border-radius: var(--radius-lg); border: 1px solid var(--color-border); box-shadow: var(--shadow-sm); margin-bottom: 40px; display: flex; flex-direction: column; gap: 20px;">
      <div style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 16px;" class="form-row">
        <div class="form-group" style="margin-bottom: 0;">
          ...
          <input type="text" id="search-input" class="form-input" placeholder="Buscar por palabra clave, ONG, Voluntario, causa u oficio..." style="background-color: var(--color-background);">
        </div>
        <div class="form-group" style="margin-bottom: 0;">
          ...
          <select id="category-select" class="form-input" style="background-color: var(--color-background);">
            ...
            <option value="medio-ambiente">Ubicación</option>
            <option value="educacion">Causas</option>
            <option value="accion-social">Oficios</option>
          </select>
        </div>
        <div class="form-group" style="margin-bottom: 0;">
          <label for="search-input" class="form-label" style="display: none;">Buscar por ubicación</label>
          <input type="text" id="search-input" class="form-input" placeholder="Buscar por ubicación..." style="background-color: var(--color-background);">
        </div>
      </div>
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
          <button class="filter-btn active" data-target="campaigns">Campañas</button>
          <button class="filter-btn" data-target="organizations">Organizaciones</button>
          <button class="filter-btn" data-target="volunteers">Voluntarios</button>
        </div>
    ...
    <div id="organizations-section" class="search-section" style="display: none;">
      <?php if (empty($organizaciones)): ?>
        <div class="empty-state">No se encontraron organizaciones registradas en este momento.</div>
      <?php else: ?>
    ...
    <div id="volunteers-section" class="search-section" style="display: none;">
      <?php if (empty($voluntarios)): ?>
        <div class="empty-state">No se encontraron voluntarios registrados en este momento.</div>
      <?php else: ?>
